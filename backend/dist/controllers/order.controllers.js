...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        const canBroadcast = (status === "out of delivery" &&
        ...
        if (canBroadcast) {
            ...
            const available = nearbyDeliveryBoys.filter((b) => !busySet.has(b._id.toString()));
            const candidates = available.map((a) => a._id.toString());
            if (candidates.length === 0) {
                await order.save();
                return res.status(200).json({
                    shopOrder,
                    message: "Order status updated but no delivery boy is available for this location",
                });
            }
                ...
                available.forEach(boy => {
                    ...
                    if (boySocketId) {
                        io.to(boySocketId).emit("newAssignment", {
                            ...
                            items: newAssignment?.order?.shopOrders?.find((so) => so._id.equals(newAssignment.shopOrderId)) || [],
                            subTotal: newAssignment?.order?.shopOrders?.find((so) => so._id.equals(newAssignment.shopOrderId))?.subTotal,
                        });
                    }
                });
            }
...
export const getDeliveryBoyAssignment = async (req, res) => {
    try {
        ...
        const formatted = assignments.map((a) => {
            const order = a.order;
            const shop = a.shop;
            const matchingShopOrder = order.shopOrders?.find((so) => String(so._id) === String(a.shopOrderId))?.shopOrderItems;
            const subTotal = order.shopOrders?.find((so) => String(so._id) === String(a.shopOrderId))?.subTotal;
            return {
                assignmentId: a._id,
                orderId: shop?._id,
                shopName: shop?.name,
...
export const getCurrentOrder = async (req, res) => {
    try {
        ...
        const assignmentLocation = assignment.assignedTo;
